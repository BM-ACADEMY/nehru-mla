  ...
  return (
    <section className="min-h-[90vh] flex items-center justify-center bg-[#FFF200]/20" style={{ backgroundImage: `url(${BlogBg})` }}>
      <motion.div
        ...
        className="max-w-md w-full bg-white shadow-2xl rounded-2xl p-8 text-center border-t-5 border-[#FF0000]"
      >
        <div className="flex justify-center">
          <img src={logo} alt="Party Logo" className="w-34 h-34 drop-shadow-lg" />
        </div>
        ...
        <p className="text-gray-700 mb-6">
          Enter your registered phone number to download your{" "}
          <span className="text-[#FF0000] font-semibold">NMK Membership Card</span>.
        </p>
        ...
        <input
          ...
          maxLength={10}
          className={`border-2 ${
            errorMsg ? "border-[#FF0000]" : "border-[#0033A0]/40 focus:border-[#0033A0]"
          } outline-none p-3 rounded-lg w-full mb-2 text-center`} 
        />
  ...
  <span className="absolute top-0 left-1/2 -translate-x-1/2 text-xs font-bold text-white">
    {progress}%
        ...
        <button
          ...
          disabled={loading}
          className={`w-full flex items-center justify-center gap-2 py-3 rounded-full text-white font-semibold text-lg transition-all ${
            loading ? "bg-gray-400 cursor-not-allowed" : "bg-[#FF0000] hover:bg-[#cc0000]"
          }`}
        >
          <FaDownload />
